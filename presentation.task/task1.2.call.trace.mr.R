...
mr=rhmr(map=map,reduce=reduce,inout=c("text","map"),ifolder="/wsc/voip/iprtp.traces",ofolder="/wsc/jrounds/voip/call.traces",jobname="create call trace database")

call.trace.mr.time<-system.time(
call.trace.mr<-rhex(mr)
)

rhsave(call.trace.mr,call.trace.mr.time,file="/wsc/jrounds/voip/records/call.trace.mr.RData")
